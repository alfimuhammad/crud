<script type="text/javascript">
	...
	$(document).on("click", ".tambah-dataRiwayat", function() {
		var id = $(this).attr("data-id");
		
		$.ajax({
			method: "POST",
			url: "<?php echo base_url('Pegawai/tambahriwayat'); ?>",
			data: "id=" +id
		})
		.done(function(data) {
			$('#tempat-modal').html(data);
			$('#tambah-riwayat').modal('show');
		})
	})
	...
	$(document).on('submit', '#form-tambah-riwayat', function(e){
		var data = $(this).serialize();

		$.ajax({
			method: 'POST',
			url: '<?php echo base_url('Pegawai/prosesTambahriwayat'); ?>',
			data: data
		})
		.done(function(data) {
			var out = jQuery.parseJSON(data);
	...
	$('#tambah-riwayat').on('hidden.bs.modal', function () {
	  $('.form-msg').html('');
	})
